...
func _physics_process(delta):
	...
	# Attacks
	if Input.is_action_just_pressed("attack_bow" + input_suffix) and can_attack_bow:
		perform_bow_attack()
	if Input.is_action_just_pressed("attack_sword" + input_suffix) and can_attack_sword:
		if is_on_floor():
...
func perform_jump_attack():
	...
	var hitbox = RectangleShape2D.new()
	hitbox.extents = Vector2(30, 30)  # Larger hitbox for slam
	var collision = CollisionShape2D.new()
	collision.shape = hitbox
	collision.position = Vector2(30 * (-1 if animated_sprite.flip_h else 1), 0)
	add_child(collision)
	# Simplified damage handling: apply damage to enemies in hitbox
	for area in get_tree().get_nodes_in_group("enemy") + get_tree().get_nodes_in_group("boss"):
		if area.get("health") != null:
			...
			if hitbox_rect.has_point(area_pos):
				area.health -= 15.0  # Higher damage for slam
	await get_tree().create_timer(0.2).timeout
	collision.queue_free()
	await get_tree().create_timer(attack_sword_cooldown - 0.2).timeout
	can_attack_sword = true
...
func update_animations(direction):
	...
	if direction != 0:
		...
		if is_on_floor() and animated_sprite.animation != "sword_attack" and animated_sprite.animation != "bow_attack" and animated_sprite.animation != "jump_attack":
